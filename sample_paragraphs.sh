...
for i in "${!array[@]}"; do
    python sample_paragraphs.py --input $PREDICTION_BASE$PREDICTION_PREFIX${array[i]} --gold $GOLD_BASE${array2[i]} --output $PREDICTION_BASE$SYSTEM_NAME$OUTPUT_PREFIX${array[i]} --num_samples 50 --mt0
done
